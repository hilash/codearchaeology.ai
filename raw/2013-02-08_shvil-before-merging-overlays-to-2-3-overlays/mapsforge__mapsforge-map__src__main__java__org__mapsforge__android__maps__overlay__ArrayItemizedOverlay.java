...
public class ArrayItemizedOverlay extends ItemizedOverlay<OverlayItem> {
	...
	public void addItem(OverlayItem overlayItem) {
 	...
 	/**
	 * Adds the given item to the overlay at the end of the list.
	 * 
	 * @param overlayItem
	 *            the item that should be added to the overlay.
	 */
	public void addItemAtTheEnd(OverlayItem overlayItem) {
		synchronized (this.overlayItems) {
			if (this.overlayItems.size() > 0) {
 				OverlayItem tmp = this.overlayItems.get(0);
 				this.overlayItems.add(tmp);
 				this.overlayItems.set(0, overlayItem);
 			} else {
 				this.overlayItems.add(overlayItem);
 			}
 		}
