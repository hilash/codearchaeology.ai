...
namespace DAL
{
    class tblSong : SqlHelper
    {
        ...
        public static int IsSongExist(string Song_Name)
        {
            // Returns Song_ID if the song is in the database.
            // else return 0.
            ...
            if ( 1 != dt.Rows.Count)
                return 0;
            else return int.Parse(dt.Rows[0]["Song_ID"].ToString());
        }

        public static DataTable SongMatches(string Song_Name)
        {
            // SELECT * FROM suppliers WHERE supplier_name LIKE '%bob%';
            string SS = "SELECT (Song_ID,Song_Name) FROM tblSong WHERE Song_Name LIKE '%" + Song_Name + "%'";
            DataTable dt = GetDataTable(SS, "tblSong");
            //PrintDataTable(dt);
            return dt;
        }
