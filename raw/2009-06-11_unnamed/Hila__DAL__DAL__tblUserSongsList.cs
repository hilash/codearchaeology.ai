...
namespace DAL
{
    public class tblUserSongsList : SqlHelper
    {
        ...
        {
            //string SS = "SELECT * FROM tblUserSongsList WHERE User_ID='" + User_ID + "' AND Song_ID='" + Song_ID + "'";
            //if (FullConnected(SS) <= 0)
            //{
            //    DAL.tblSong.AddSongPOP(Song_ID, User_ID);
            //    return FullConnected("INSERT INTO tblUserSongsList(User_ID, Song_ID) VALUES('" + User_ID + "','" + Song_ID + "')");
            //}
            //return 0;

            //return -100 if there else 1
            //return FullConnected("INSERT INTO tblUserSongsList(User_ID, Song_ID) VALUES('" + User_ID + "','" + Song_ID + "')");

            //return FullConnected(SS);

            if (FullConnected("INSERT INTO tblUserSongsList(User_ID, Song_ID) VALUES('" + User_ID + "','" + Song_ID + "')") == 1)
            {
        ...
        public static int DeleteUserSongsList(int User_ID, int Song_ID)
        {
            ...
            if (FullConnected("DELETE FROM tblUserSongsList WHERE User_ID='" + User_ID + "' AND Song_ID='" + Song_ID + "'") == 1)
            {
                DAL.tblSong.AddSongPOP(Song_ID, -1);
                return 1;
            }
