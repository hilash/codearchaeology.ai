...
/* The initial expanding function.  The hash function is defined over an
   ...
   buffer - thanks to Colin Plumb,  for this
   optimization.

   The updated SHS changes the expanding function by adding a rotate of 1
   bit.  Thanks to Jim Gillogly, , and an anonymous contributor
...
MD5_CTX *mdContext;
{
  ...
  for (i = 0; i < 16; i++)
  {
	...
	sprintf(stri+len,"%02x", mdContext->digest[i]);
	len=strlen(stri);
  }
...
SHA_CTX sha;
	...
	len=0;
	SHAInit(&sha);
	SHAUpdate(&sha, SEED, strlen(SEED));
	SHAFinal(digest, &sha);
	for (i = 0; i < 20; i++)
	{
		/*printf("%02x", digest[i]);*/
		sprintf(SHA1+len,"%02x", digest[i]);
		len=strlen(SHA1);
	}
	...
	len=0;
	SHAInit(&sha);
	SHAUpdate(&sha, SHA1, strlen(SHA1));
	SHAFinal(digest, &sha);
	for (i = 0; i < 20; i++)
	{
		/*printf("%02x", digest[i]);*/
		sprintf(SHA1_2+len,"%02x", digest[i]);
		len=strlen(SHA1_2);
	}
