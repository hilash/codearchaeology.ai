...
{
    /// <summary>
    /// Parser for the ABC file format
    /// based on http://stephen.homedns.org/public/ABCquickRefv0_5.pdf
    /// </summary>
    static public class AbcParser
    {
        /// <summary>
        /// Parse ABC file to list of Notes
        /// </summary>
        /// <param name="abcFilePath">Path of the ABC file to parse</param>
        /// <returns>List of Notes (programm class) in the ABC file</returns>
        static public RecordingData AbcToNotes(string abcFilePath)
        {
            RecordingData recordingData = new RecordingData();
            NotesDB notesDB = new NotesDB();

            if (!File.Exists(abcFilePath))
            {
        ...
        static private void ParseNotesLine(RecordingData recordingData, string line, NotesDB notesDB)
        {
            ...
            for (int i = 0; i < line.Length; i++)
            {
                ...
                {
                    if (line[i] == '^') { isSharp = true; }
                    if (line[i] == '_') { isFalt = true; }
                }
                else
                {
                    isSharp = false;
