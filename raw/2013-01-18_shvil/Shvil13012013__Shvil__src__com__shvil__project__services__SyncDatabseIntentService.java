 ...
 *
 */
public class SyncDatabseIntentService extends IntentService {
	...
	public SyncDatabseIntentService() {
		super("SyncDatabseIntentService");
	...
	protected void onHandleIntent(Intent arg0) {
		...
		// if unique use ID not found on preferences
		/*
		if (false){
			getUniqueUserId();
		}*/
	...
	private void SyncDB() {
		...
		String jsonRecordsToUpdate = postLocalUpdatedDbRecords();
		
		// retrieve the diff between the local and the server DB
		// and update the local DB
		retrieveSeverUpdatedDbRecords(jsonRecordsToUpdate);	
	}
	
	private String postLocalUpdatedDbRecords() {
		
		String json = getJsonOfRecordsToUpdate("TableName");
	...
	private String getJsonOfRecordsToUpdate(String tableName) {
		String json;
		Gson gson = new Gson();
		
		// TODO - sync accsees to DB

		AngelDao angelDao = myApplication.daoSession.getAngelDao();
		Collection<Angel> angels = angelDao.loadAll();
		Type collectionType = new TypeToken<Collection<Angel>>(){}.getType();
		json = gson.toJson(angels, collectionType);
		return json;
	}

	private void retrieveSeverUpdatedDbRecords(String jsonRecordsToUpdate) {

		Gson gson = new Gson();
		Type collectionType = new TypeToken<List<Angel>>(){}.getType();
		List<Angel> angels = gson.fromJson(jsonRecordsToUpdate, collectionType);
		// TODO - sync accsees to DB
