...
namespace Recorder
{
    public partial class Form1 : Form
    {
        ...
        const uint SamplesPerSec = 11025; //4000 give results

        // UI elements
        int pitchTrackBarDelta;
        ...
        private void printGraph(Graphics graph, int[] wave, int maxAmplitude, Color background, Pen foreground, bool stretch_graph)
        {
            ...
            if (true == stretch_graph)
            {
                stretch = (float)width / (float)wave.Length;
                numberOfSamplesToDraw = wave.Length;
            }
            else
            {
                stretch = 1;
                numberOfSamplesToDraw = Math.Min(width, wave.Length);
            }
        ...
        private void recording_loop()
        {
            try
            {
                ...
                while (is_recording)
                {
                    ...
                    int maxPitch = furierTransform.preformTranform(transformType, waveArray, waveArrayFrequency, (int)NumberOfSamples);
                    if (!is_recording) { return; }

                    // print time domain
                    int[] tmpArray = new int[waveArray.Length];
                    for (int i = 0; i < waveArray.Length; i++) { tmpArray[i] = (int)waveArray[i]; }
                    printGraph(timeDomainGraphics, tmpArray, byte.MaxValue, Color.FromArgb(39, 40, 34), pen, false);
                    if (!is_recording) { return; }
                    ...
                    printGraph(frequencyDomainGraphics, tmpArray, tmpArray.Max(), Color.FromArgb(39, 40, 34), pen, true);
                    if (!is_recording) { return; }

                    // update Pitch based-on user's choice
                    maxPitch += pitchTrackBarDelta;
        ...
        private void useFFT_CheckedChanged(object sender, EventArgs e)
        {
            transformType = FurierTransform.TRANSFORM.FFT;
        }

        private void useDFT_CheckedChanged(object sender, EventArgs e)
        {
        ...
        {
            pitchTrackBarDelta = pitchTrackBar.Value;
        }
    }
