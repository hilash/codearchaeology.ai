...
namespace Recorder
{
    class FingeringChart
    {
        ...
        public void Draw(Graphics graphics, Brush brush)
        {
            ...
            SolidBrush musicSheet = new SolidBrush(Color.RosyBrown);
            SolidBrush note = new SolidBrush(Color.Blue);
            noteDrawer = new NoteDrawer();
            for (int i = 0; i < notes.Count; i++)
            {
                noteDrawer.Draw(graphics, background, musicSheet, note, notes[i]);
            }
        }
    }
}
