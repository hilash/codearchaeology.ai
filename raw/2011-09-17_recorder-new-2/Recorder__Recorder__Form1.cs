...
namespace Recorder
{
    public partial class Form1 : Form
    {
         ...
         * Forms Varibles
         ***************************************************************************************/
        const uint NumberOfSamples = 1024;//4096;
        const uint SamplesPerSec = 22050;//88200;
        ...
        // threads synchronization
        ThreadStart playRecordingThreadStart;
        Thread playRecordingThread;
        ...
        public Form1()
        {
            try
            {
                ...
                noteDetector = new NotesDetector();
            ...
            catch (Exception ex)
            {
                ...
                SetControlEnabled(buttonStartPlayingRecord, false);
            }
        }
        ...
        private void RecordingLoop()
        {
            try
            {
                ...
                while (isRecording)
                {
                    ...
                    SetControlText(textBoxNote, currentNote.NoteDescription);
        ...
        private void StopRecord()
        {
            try
            {
                ...
                SetControlEnabled(buttonStartPlayingRecord, true);
            }
            catch (Exception ex)
            {
                string error_message;
        ...
        private void buttonStartRecord_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                // open the waveform-audio input device, and start recording
                Wave.startDevice((uint)(comboBoxInputDevices.SelectedIndex - 1), @"C:\temp\bla2.wav");
        ...
        {         
            try
            {
                ThreadStart playRecordingThreadStart = new ThreadStart(PlayRecording);
                Thread playRecordingThread = new Thread(playRecordingThreadStart);
        ...
        {
            MIDI.MIDI.PlayNotes(NotesPlayed);
            /*
            try
            {
                SetControlEnabled(comboBoxInputDevices, false);
                SetControlEnabled(buttonStartRecord, false);
                SetControlEnabled(buttonStopRecord, false);
                SetControlEnabled(buttonPlayRecord, false);
                ...
                SetControlEnabled(comboBoxInputDevices, true);
                SetControlEnabled(buttonStartRecord, true);
                SetControlEnabled(buttonPlayRecord, true);
            }
            catch (Exception ex)
            {
                string error_message;
