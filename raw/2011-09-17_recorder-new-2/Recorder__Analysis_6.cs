...
namespace Analysis
{
    ...
    public class FurierTransform
    {
        double[] m_REX;
        double[] m_IMX;

        public FurierTransform(uint initialNumberOfSamples)
        {
        ...
        public int preformTranform(TRANSFORM transform, byte[] inputBuffer, double[] outputBuffer, int numberOfSamples)
        {
            int maxPitch = 0;
            ...
            {
                Array.Resize(ref m_REX, numberOfSamples);
                Array.Resize(ref m_IMX, numberOfSamples);
            }

            if (TRANSFORM.DFT == transform)
            {
                maxPitch = DFT(inputBuffer, m_REX, m_IMX, outputBuffer, numberOfSamples);
            }
            else if (TRANSFORM.FFT == transform)
            {
                Array.Copy(inputBuffer, m_REX, numberOfSamples);
                Array.Clear(m_IMX, 0, numberOfSamples);
                maxPitch = FFT(m_REX, m_IMX, outputBuffer, numberOfSamples);
            }
            else
            {
